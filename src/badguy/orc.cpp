...
Orc::Orc(const std::string& filename) :
	BadGuy(filename)
{
	m_weapon = WeaponSet::current()->get(AK47_BADGUY).clone(this);
	m_weapon->set_offset(m_weapon->get_bounding_box().get_middle());
	set_action("idle-right");
}

void Orc::update(float dt_sec) {
	if (m_health <= 0) {
...
void Orc::draw(DrawingContext& drawing_context) {
	if (m_health <= 0) {
...
void Orc::active_update(float /* dt_sec */) {
...
void Orc::inactive_update(float /* dt_sec */) {
...
std::unique_ptr<BadGuy> Orc::from_file(const ReaderData* data) {
	std::string filename;
	if (!data->get("filename", filename)) {
		throw std::runtime_error("Missing badguy data");
	}
	auto badguy = std::make_unique<Orc>(data->m_parent_path + filename);
...
bool Orc::is_boss() const { return false; }

std::unique_ptr<BadGuy> Orc::clone(const Vector& pos) const {
	auto badguy = std::make_unique<Orc>(m_sprite_name);
