...
void Necromancer::draw(DrawingContext& drawing_context) {
	if (m_health <= 0) {
...
void Necromancer::activated() {}

void Necromancer::deactivated() {
...
void Necromancer::active_update(float /* dt_sec */) {
...
void Necromancer::inactive_update(float /* dt_sec */) {
...
std::unique_ptr<BadGuy> Necromancer::from_file(const ReaderData* data) {
	std::string filename;
	if (!data->get("filename", filename)) {
		throw std::runtime_error("Missing badguy data");
	}
	auto badguy = std::make_unique<Necromancer>(data->m_parent_path + filename);
...
bool Necromancer::is_boss() const { return false; }

std::unique_ptr<BadGuy> Necromancer::clone(const Vector& pos) const {
	auto badguy = std::make_unique<Necromancer>(m_sprite_name);
	...
	badguy->m_timer_ultimate.start(ULTIMATE, true);
	badguy->m_timer_ultimate.pause_with_previous();
	return badguy;
}